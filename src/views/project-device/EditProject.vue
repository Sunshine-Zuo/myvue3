...
<script setup>
import { editProject, getProjectInfo, getUserList } from "@/utils/api.js";
...
const textToCode = {};
pcC.forEach(province => {
  textToCode[province.name] = province.code;
  province.children && province.children.forEach(city => {
    textToCode[city.name] = city.code;
    city.children && city.children.forEach(area => {
      textToCode[area.name] = area.code;
    });
  });
});
let CascaderProps = {
...
const _getProjectInfo = () => {
  ...
  getProjectInfo({ id: projectId }).then(res => {
    if (res.code == 200) {
      let data = res.data
      if (data.responsibleIds) data.responsibleIds = data.responsibleIds.split(',')
      if (data.warningIds) data.warningIds = data.warningIds.split(',')
      if (data.province) selectedAddress1.value = [textToCode[data.province], textToCode[data.city]]
      ruleForm.value = data
    }
  })
}
